            ...
            <div class="card {!! $printCard !!}">
                ...
                <div class="card-footer">
                    ...
                    @elseif (in_array($viewType, ['show']) && $permission['edit'] == true)
                        <a href="<?php echo route('admin.' . $thisRoute . '.approved', $data->{$masterId}) ?>"
                           class="mb-2 mr-2 btn btn-primary" title="{{ __('general.approved') }}">
                            <i class="fa fa-check"></i><span class=""> {{ __('general.approved') }}</span>
                        </a>
                    @endif
                    <a href="<?php echo route('admin.' . $thisRoute . '.rejected' , $data->{$masterId}) ?>" class="mb-2 mr-2 btn btn-danger"
                       title="{{ __('general.rejected') }}">
                        <i class="fa fa-window-close"></i><span class=""> {{ __('general.rejected') }}</span>
                    </a>
                </div>
                {{ Form::close() }}
            </div>
