                    ...
                    <div class="card-header">
                        <a href="<?php echo route('admin.' . $thisRoute . '.create') ?>" class="mb-2 mr-2 btn btn-primary"
                           title="@lang('general.create')">
                            <i class="fa fa-edit"></i> @lang('general.ajukan_pemuktahiran')
                        </a>
                    </div>
            @endif
...
@section('script-bottom')
    ...
    <script type="text/javascript">
        ...
        function askingReject(curr) {
        let alasan = prompt("alasan untuk menolak pemuktahiran ini?");
        if(alasan != null) {
            let href = $(curr).attr('href');
            $(curr).attr('href', href + '?alasan=' + alasan);
            return true;
        }
        else {
            return false;
