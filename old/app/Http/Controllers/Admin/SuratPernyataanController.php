...
use App\Codes\Models\SuratPernyataanInfo;
...
class SuratPernyataanController extends _CrudController
{
    public function __construct(Request $request)
    {
        $passingData = [
            ...
            'tanggal' => [
                ...
                'type' => 'datepicker',
                'lang' => 'Tanggal Surat Pernyataan'
            ],
            'nomor' => [
                'validate' => [
                    'create' => 'required|max:191',
                    'edit' => 'required|max:191'
                ],
                'lang' => 'Nomor',
                'value' => 'M.390.KP.04.12. TAHUN 2012'
            ],
            'tanggal_mulai' => [
                'validate' => [
                    'create' => 'required',
    ...
    public function dataTable()
    {
        ...
        if ($getFlag == 1) {
            $this->listView['dataTable'] = env('ADMIN_TEMPLATE').'.page.surat_pernyataan.list_button_atasan';
            $getStaff = Staffs::where('user_id', $userId)->first();
            $getBawahan = Staffs::where('staff_id', $userId)->pluck('user_id');
            if ($getBawahan) {
                    $builder = $builder->whereIn('user_id', $getBawahan);
...
//        $data['created_by'] = session()->get('admin_name');
    ...
    public function approve($id)
    {
        ...
        $createPdf = $this->createPDF($id, $userId);

        $getData->approved = 2;
        $getData->supervisor_id = $userId;
        $getData->pdf = $createPdf['name'];
        $getData->pdf_url = $createPdf['location'];
        $getData->save();

        $getData->getKegiatan()->update([
            'approved' => 2
        ]);
        ...
        if($this->request->ajax()){
            ...
            session()->flash('message', __('general.success_approve', ['field' => 'Surat Pernyataan '.$getData->nomor]));
            session()->flash('message_alert', 2);
    ...
    public function reject($id)
    {
        ...
        $list_kegiatan = $getData->getKegiatan()->update([
            'approved' => 9
        ]);
